...
from routers.jobdes import get_db, JOBDES
...
def get_candidate_resume(candidate_id: str) -> Optional[ResumeData]:
    ...
    try:
        ...
        print(f"[Warning] Could not load candidate JSON from '{candidate_id}' from S3: {e}")
        return None
...
def rank_candidate(
    ...
    top_k: int = Query(10, description="Number of top candidate to return in leaderboard"),
    ...
    for candidate_id, vector_sim in search_results:
        resume_obj = get_candidate_resume(candidate_id)
        if resume_obj:
