...
def build_json_to_text(jsontext: ResumeData):
    ...
    for exp in jsontext.work_experience:
        bullets = " ".join(exp.bullet_points)
        exp_list.append(f"{exp.title} at {exp.company}: {bullets}")
    ...
    for proj in jsontext.projects:
        ...
        bullets = " ".join(proj.bullet_points)
        proj_list.append(f"{proj.name}{tech}: {bullets}")
    ...
    certs_text = f"Certifications: {', '.join(jsontext.certifications)}" if jsontext.certifications else ""
